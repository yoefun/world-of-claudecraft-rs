...
(`a3e5e9596a8e9e7d37b5b23efbbb0f2cd846c0c9`). Parity target: **`party-depth`**.
...
## What works in 1.14.0 (party-depth)
...
- Talk to NPCs (E), quests (abandon **L+X**, share **L+Y**, daily/explore/escort, choice rewards **1/2/3**), vendor buyback, repair, trainers, hearth, combat, party/chat (**G** invite · **P** party · frames · ready check · park-safe roster · classic XP split), Need/Greed loot (1/2/3 rolls; [ ] loot mode)
...
- Client panels: talents / bank / mail / market / bags (equip·use·sell); **character sheet** (C: 9 slots, quality, MH enchant, AP/armor/SP); **minimap** + **world map** (M)
- Class armor caps, two-hand occupancy, warrior/rogue dual-wield, two rings, catalog quality, MH oils; stamina → HP, spell power on heals/spells; independent loot piles
...
Controls: **WASD** move, **Space** jump / swim hop / fly up, **Ctrl** descend (swim/fly), **V** travel flight toggle, **mouse** look (hold right), **left click** / **F** attack, **Tab** cycle target, **G** invite (player target), **P** party (**O** accept / **X** leave / **Y** promote / **-** kick / **R** ready) · party frames, **1–5** abilities (or Need/Greed/Pass while rolling), **T** pet summon/dismiss (hunter/warlock), **Esc** clear target / stop attack / release cursor, **E** interact/loot, **B** bags (**Q** equip / **F** use / **V** sell junk at vendor), **L** quests, **C** character, **N** talents (**1–3** spend / Y first available / R respec), **K** bank (**G** deposit junk / **H**/**1–9** withdraw / **J**/**Y** copper vault), **I** mail (**P** collect), **M** world map, **U** market (**L** list / **O** buy / **X** cancel), **[**/**]** party loot mode. Title: **1/2** or click Offline|Online, **Enter**/Continue. Offline create: **click** or **←/→** class grid, type name, **Enter**. Online login: **Tab** field, **F2**/tabs login|register, **Enter**/Sign in (register asks for password confirm). Char select: click roster or **↑/↓**, **Enter world**, **N**/New character (class grid), **D**/Delete (confirm twice), **Esc** logout.
...
See [`docs/ROADMAP.md`](docs/ROADMAP.md). **Shipped:** `1.14.0` / `party-depth`. Manual demo: [`docs/parity/DEMO.md`](docs/parity/DEMO.md).
