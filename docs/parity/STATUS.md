...
**Current rewrite:** `1.26.1` / `class-depth` (PROTOCOL_REV=11).
...
## Client asset compatibility (`1.26.1`) — done

The patch release preserves gameplay and protocol behavior while making the packaged GLB runtime closure loadable by Bevy 0.16.

| Subsystem | Status | Notes |
| --- | --- | --- |
| 23 mapped GLB scenes | done | Repacked without meshopt / mesh quantization declarations |
| WebP textures | done | Standard glTF texture sources with Bevy WebP decoding |
| Wild-boar animation data | done | Invalid morph-weight channels normalized during repacking |
| Protocol / upstream pin | unchanged | Rev 11; upstream 0.31.0 at the existing pinned commit |
...
| Subsystem | Status | Notes |
| --- | --- | --- |
| Version / upstream pin | done | `1.26.1` / class-depth (upstream still 0.31.0) |
