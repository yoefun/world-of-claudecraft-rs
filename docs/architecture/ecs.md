...
Player columns include `Hearth` (`zone_id`, bind `x`/`z`, `ready_tick`) for innkeeper binding and tick-based hearthstone cooldowns, `Reputation` (`values: faction_id → i32`) for hub faction standing, and `Riding` (`rank`, `known`, `last_id`, `active_id`) for mount training and summon state. See `crates/woc-sim/src/ecs/components.rs`.
